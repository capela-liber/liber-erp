...
@tagged('post_install', '-at_install', 'liber_aged_payable')
class TestAclSomenteLeitura(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.somente_leitura = Users.create(dict(
            ...
            login='readonly_pagar@liber.test',
            group_ids=[(6, 0, [
                cls.env.ref('base.group_user').id,
                cls.env.ref('account.group_account_readonly').id,
            ])]))
        cls.contador = Users.create(dict(
            base, name='Contador pleno', login='pleno_pagar@liber.test',
            group_ids=[(6, 0, [
                cls.env.ref('base.group_user').id,
    ...
    def test_somente_leitura_le_o_relatorio(self):
        ...
        env['liber.aged.payable'].check_access('read')
        env['liber.aged.payable'].search([], limit=5).mapped('display_name')
    ...
    def test_ninguem_escreve_no_relatorio(self):
        ...
        for user in (self.somente_leitura, self.contador):
            ...
            with self.assertRaises(AccessError):
                env['liber.aged.payable'].check_access('write')
    ...
    def test_sem_grupo_de_contabilidade_nao_le(self):
        ...
        cru = Users.create({
            'name': 'Sem contabilidade', 'login': 'cru_pagar@liber.test',
